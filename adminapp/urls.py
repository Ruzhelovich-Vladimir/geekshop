...
urlpatterns = [
    ...
    path('admin/orders/list/', adminapp.AdminOrdersList.as_view(),
         name='admin_orders_list'),
]

# Не могу понять почему не корректно подтягивается шаблон "admin_orders_list.html", сдаю как есть, буду разбираться.
